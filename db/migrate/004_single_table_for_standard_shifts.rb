class SingleTableForStandardShifts < ActiveRecord::Migration
  def self.up
    ...
    execute "INSERT INTO shifts (type, start_time, end_time, splitable, mergeable, resizable, coverage_type_id, job_id, meeting_id, schedule_id, weekday_id, worker_id) (SELECT 'shift', start_time, end_time, splitable, mergeable, resizable, coverage_type_id, job_id, meeting_id, schedule_id, weekday_id, worker_id FROM standard_shifts)"
    execute "INSERT INTO shifts (type, meeting_name, meeting_date, start_time, end_time, splitable, mergeable, resizable, coverage_type_id, frequency_type_id, schedule_id, weekday_id ) (SELECT 'meeting', name, meeting_date, start_time, end_time, splitable, mergeable, resizable, coverage_type_id, frequency_type_id, schedule_id, weekday_id FROM meetings)"
    ...
    execute 'ALTER TABLE meetings_workers ADD CONSTRAINT meetings_workers_meetings FOREIGN KEY ( shift_id ) REFERENCES shifts( id ) ON DELETE CASCADE'
  end
