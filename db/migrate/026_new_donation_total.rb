class NewNewDonationTotal < ActiveRecord::Migration
  def self.up
    create_view( :v_donation_totals,
                 "select d.id, sum(p.amount_cents) from donations as d " +
    ...
    create_view( :v_donations, "select d.*, v.total_paid, " +
                 "CASE WHEN (v.total_paid > d.reported_required_fee_cents) THEN d.reported_required_fee_cents ELSE v.total_paid END, " +
                 "CASE WHEN (v.total_paid < d.reported_required_fee_cents) THEN 0 " +
                 "ELSE (v.total_paid - d.reported_required_fee_cents) END " +
