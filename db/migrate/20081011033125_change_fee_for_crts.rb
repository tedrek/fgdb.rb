class ChangeFeeForCrts < ActiveRecord::Migration
  def self.up
    GizmoType.connection.execute("UPDATE gizmo_types SET description='Old Fee Television', name='old_television' WHERE description='Television'")
    ...
    {:television => ["Television", 4], :sys_with_monitor => ["Sys w/ monitor", 1], :crt => ["CRT", 2]}.each do |k,v|
