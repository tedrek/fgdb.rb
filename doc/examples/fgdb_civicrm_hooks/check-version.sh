...
LATEST=$(curl -s "http://sourceforge.net/projects/civicrm/files/civicrm-stable/" | sed -n '/Looking for the latest version/ {n;n;n;p;}' | sed -n -r '/title=/ {s/.*title="([^"]+)"/\1/; p;}' | cut -d / -f 3)
