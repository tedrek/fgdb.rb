...
class UserTests < Test::Unit::TestCase
	...
	def test_021_unauthorized_users 
		guest = nil
		assert_nothing_raised       { guest = FGDB.login( 'guest' ) }
		assert                      guest
	...
	def test_030_login
        ...
        assert_raises( FGDB::LoginError ) { FGDB.login( 'god', 'blah' ) }
        ...
        assert_raises( FGDB::LoginError) { FGDB.login( 'god', 'sex' ) }
        assert_nothing_raised { user = FGDB.login( 'god', 'blah' ) }
