require File.join(File.dirname(__FILE__), '..', 'lib', 'export_fixtures.rb')
...
desc 'use rake db:fixtures:import_all to import all YAML test fixtures for all of the models from data in an existing database. Defaults to development database. Set RAILS_ENV to override. '
namespace :db do
  namespace :fixtures do
    task :import_all => :environment do
      ActiveRecord::Base.establish_connection
      Dir.glob(File.join(RAILS_ROOT,'test','fixtures',"*.yml")).each do |f|
        model_name = f.gsub(File.join(RAILS_ROOT,'test','fixtures', ''), '').gsub('.yml', '').classify
        import_model_fixture(model_name)
      end
    end
  end
