...
class PaymentMethodTest < ActiveSupport::TestCase
  ...
  def test_raw_validation
    ...
    if REQ_ATTR_NAMES.blank?
      ...
      REQ_ATTR_NAMES.each {|attr_name| assert payment_method.errors.invalid?(attr_name.to_sym), "Should be an error message for :#{attr_name}"}
    end
  end
	...
	def test_validates_presence_of
   	REQ_ATTR_NAMES.each do |attr_name|
    	...
    	assert payment_method.errors.invalid?(attr_name.to_sym), "Should be an error message for :#{attr_name}"
    end
 	end
