module HtmlHelper
  ...
  def date_or_date_range_picker(obj_name, field_name)
    ...
    date_types << ['quarterly', select(obj_name, field_name + '_quarter', (1..4).map{|x| ["Q" + x.to_s, x]})+ select_year(obj.send(field_name + '_year_q'), :prefix => obj_name, :field_name => field_name + '_year_only', :start_year => 2000, :end_year => Date.today.year)]
