  ...
  def VolunteerDefaultShift.generate(start_date, end_date, gconditions = nil, skip_these = [])
    ...
    (start_date..end_date).each{|x|
      ...
      shifts.each{|ds|
        ...
        myl = VolunteerEvent.find(:all, :conditions => ["date = ?", x], :include => [:volunteer_shifts => :assignments]).map{|y| y.id == ve.id ? ve : y}.map{|y| y.volunteer_shifts.map{|q| q.assignments}}.flatten.select{|y| (ds.volunteer_task_type_id.nil? ? (ds.volunteer_default_event.description == y.volunteer_event.description) : ((ds.volunteer_task_type_id == y.volunteer_shift.volunteer_task_type_id))) and ((conflict_start_time >= y.start_time and conflict_start_time < y.end_time) or (conflict_end_time > y.start_time and conflict_end_time <= y.end_time) or (conflict_start_time < y.start_time and conflict_end_time > y.end_time))}.map{|y| y.volunteer_shift.slot_number}
