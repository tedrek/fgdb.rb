class Roster < ActiveRecord::Base
  ...
  def auto_generate(from, to)
    ...
    begin
      ...
      results = conflicts.map{|x| "On #{x[0]}, #{x[1].contact.display_name} was not successfully scheduled for #{x[1].slot_type_desc} as they have the following conflicting shifts: #{x[2].map{|x| x.description}.join(" ")}"}
