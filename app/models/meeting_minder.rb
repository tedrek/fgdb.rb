class MeetingMinder < ActiveRecord::Base
  ...
  def minder_variables(today)
    ...
    {:meeting_name => meeting.meeting_name, :meeting_date => (today + days_before).strftime("%a, %B %d %Y"), :days_before => days_before, :todays_date => today.strftime("%a, %B %e %Y"), :last_meeting_date => meeting.last_meeting(today), :meeting_time => meeting.start_time.strftime("%I:%M %p").downcase}
  end
