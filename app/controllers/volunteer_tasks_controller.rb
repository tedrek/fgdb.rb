class VolunteerTasksController < ApplicationController
  ...
  def get_required_privileges
    a = super
    a << {:privileges => ["contact_#{_get_contact_id.to_s}", 'role_volunteer_manager']}
    a << {:only => ["/everybody"], :privileges => ['role_volunteer_manager']}
    return a
  end
  ...
  def get_contact_id
    ((params && params[:contact_id] && params[:contact_id].to_i) || (params && params[:volunteer_task] && params[:volunteer_task][:contact_id] && params[:volunteer_task][:contact_id].to_i) || (has_privileges("role_volunteer_manager") ? nil : (@current_user && @current_user.contact_id)))
  end
