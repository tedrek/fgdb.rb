...
class ApplicationController < ActionController::Base
  ...
  def do_find_problems_report(klass, column, weeks, dates = [], mode = "ws", schedule_id = nil)
    ...
    @workers = []
    @workers_h = {}
    all_scheduled = klass.find(:all, :conditions => ["#{mode == "ws" ? "" : " shift_date IS NULL AND (ineffective_date IS NU'LL OR ineffective_date >= '" + Date.today.to_s + "') AND "} #{column} >= ? AND #{column} <= ? AND #{mode == "ws" ? "kind" : "type"} NOT LIKE 'Unavailability'", @weeks.first, (@weeks.last+6)], :order => "#{column} ASC")
